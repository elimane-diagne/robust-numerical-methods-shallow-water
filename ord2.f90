program hydrostatic_reconstruction_second_order
  ...
  ! Paramètres globaux
  integer, parameter :: n_cells = 200, n_steps = 20
  ...
  do t_step = 1, n_steps
     ...
     ! Sauvegarde des résultats tous les 10 pas de temps
...
contains
  ...
  subroutine initialize(h, u, z, x, dx, n_cells)
    ...
    do i = 1, n_cells
        x(i) = (i - 1) * dx
        z(i) = 0.5
        h(i) = 1.0 - z(i)  ! Hauteur initiale H_0 = 1.0
        u(i) = 0.0
    end do
  ...
  subroutine save_results(t_step, x, h, z, n_cells)
    ...
    write(filename, "(A,I0)") "results_code_order2_step_", t_step
